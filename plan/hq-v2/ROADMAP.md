...
| K — HITL pause-policy + hỏi-user (#3, CD-3) | `plan/hq-v2/phase-k/` | 📋 Long-plan soạn xong (2026-06-04), chưa thực thi. 3 sub-phase / 6 session: K.A schema `pause`+validate+`user_answer` reserved+fixture `ask-demo`+ask-pause executor · K.B resume `-Answer` re-run node đã hỏi+`pause:always` run-then-gate · K.C dispatcher `-Answer`+status+selftest #11 (10→11)+docs/viz+done-gate 4/4. 4 quyết định chốt (D-K1 marker `ASK_USER:` · D-K2 re-run node · D-K3 pause-policy đầy đủ trên worker · D-K4 state `awaiting_input`+event `awaiting`+kind). Xem `phase-k/PLAN.md`. |
