...
| I — Tối ưu token chi nhánh (#2) | `plan/hq-v2/phase-i/` | 🟡 Long-plan soạn xong (2026-06-04), chưa thực thi. 4 sub-phase / 9 session: I.A đo lường (bắt `.usage` JSON + `run.ps1 tokens` + baseline) · I.B model-tier+siết-template · I.C artifact-by-ref + handoff-output đích + conditional-trim · I.D prompt-caching + REAL-RUN report (user-gate cuối). 3 QĐ chốt: D-I1 layer cả hai (artifact-ref nền + trim single-consumer) · D-I2 đủ 6 hạng mục · D-I3 real-run user-gate 1 session cuối. Mock-path + regression bất biến. |
...
| J2 — Gộp worker+router (routing theo cạnh, CD-2 ext) | `plan/hq-v2/phase-j2/` | ✅ DONE (2026-06-04). 2 session: J2.1 executor edge-based (`Test-NodeBranches`)+migrate fixtures/patterns · J2.2 validate reject type:router+viz/graph+docs. Self-mod chain. selftest 10/10 PASS. |
| K — HITL pause-policy + hỏi-user (#3, CD-3) | `plan/hq-v2/phase-k/` | ✅ **DONE (2026-06-04)**. 3 sub-phase / 6 session: K.A schema `pause`+validate+`user_answer` reserved+fixture `ask-demo`+ask-pause executor · K.B resume `-Answer` re-run node đã hỏi+`pause:always` run-then-gate · K.C dispatcher `-Answer`+status+selftest #11 (10→11)+docs/viz+done-gate 4/4. Deliverable: `pause: none\|always\|ask` trên node worker + trạng thái `awaiting_input` + `resume -Answer` (re-run node) + event `awaiting` kind=input/approval + fixture `examples/ask-demo/` + `examples/always-demo/` + selftest 11/11. Mock-path + graph cũ + approval-node bất biến. 4 quyết định chốt: D-K1 marker `ASK_USER:` · D-K2 re-run node · D-K3 pause-policy đầy đủ trên worker · D-K4 state `awaiting_input`+event `awaiting`+kind. |
