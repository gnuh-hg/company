# stamp.ps1 — build-time: fragment research-gather → workflow.json (demo wrapper).
# Chạy: pwsh examples/p-research-gather/stamp.ps1   (sinh workflow.json cạnh script này)
#
...
$prefix   = 'rg'
$fragment = Read-Json (Join-Path $projDir '../../patterns/research-gather.json')
$expanded = Expand-Pattern $fragment $prefix

# Host bind agent mỗi node theo convention agents/<stamped-id>.md
# (router agent basename = node id 'rg_gate' → khớp ENGINE_MOCK_ROUTER).
...
$workflow = [ordered]@{
    name      = 'p-research-gather'
    entry     = "${prefix}_researcher"
    max_steps = 10
    nodes     = @($nodes)
...
Write-Host "✓ stamped p-research-gather: $($workflow.nodes.Count) node / $(@($workflow.edges).Count) edge" -ForegroundColor Green
