# stamp.ps1 — build-time: fragment re-plan-loop → workflow.json (demo wrapper).
# Chạy: pwsh examples/p-re-plan-loop/stamp.ps1   (sinh workflow.json cạnh script này)
#
...
# re-plan-loop có cycle (verdict fail/clarify → planner) → max_steps là cầu dao bắt buộc.
...
$prefix   = 'rp'
$fragment = Read-Json (Join-Path $projDir '../../patterns/re-plan-loop.json')
$expanded = Expand-Pattern $fragment $prefix

# Host bind agent mỗi node theo convention agents/<stamped-id>.md
# (router agent basename = node id 'rp_verdict' → khớp ENGINE_MOCK_ROUTER).
...
Write-Host "✓ stamped p-re-plan-loop: $($workflow.nodes.Count) node / $(@($workflow.edges).Count) edge" -ForegroundColor Green
