...
**Input** — `{{story}}` (acceptance criteria + user story) + `{{fe_ref}}` (đường dẫn tới file frontend implementation — **dùng Read tool để đọc nội dung trước khi test**).

**Cách dùng artifact-by-reference:**
```
{{fe_ref}} là đường dẫn tuyệt đối tới fe.txt.
Dùng Read tool: Read({{fe_ref}}) để đọc toàn bộ hoặc chọn phần cần thiết.
```
