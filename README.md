...
| | `resume` | `resume <proj> [-Mock] [-Model m] [-Decision label] [-Answer text]` | Tiếp tục run dở / `failed` / `awaiting` / `awaiting_input`; `-Decision` bơm quyết định duyệt; `-Answer` bơm câu trả lời (pause:ask) |
...
| **Advanced** | `selftest` | `selftest [all]` | Chạy bộ test engine (7 stamp + mem-demo + approval-demo + branchy/2-part + ask-demo/done-gate); **11 mục, exit = số mục fail** |
...
#### Pause-policy — chính sách dừng trên node worker (Phase K)

Mỗi node worker có thể khai field `pause` (vắng = `none`):

| `pause` | Hành vi |
|---|---|
| `none` | Chạy thẳng, không dừng (mặc định, bất biến) |
| `always` | Chạy agent XONG (ghi `output_key`) → dừng **`awaiting`** kind=approval → `resume -Decision approve` để tiếp. Khác `type:approval`: always **có gọi model + có output_key**. |
| `ask` | Agent tự quyết lúc chạy: nếu thiếu thông tin → in marker `ASK_USER: <câu hỏi>` → engine dừng **`awaiting_input`** → người trả lời → engine tiêm `{{user_answer}}` → **re-run chính node đó**. |

```json
{ "id": "clarify", "pause": "ask",    "input": "{{user_request}}\nCâu trả lời: {{user_answer}}", "output_key": "spec" }
{ "id": "review",  "pause": "always", "input": "{{spec}}", "output_key": "review_out" }
```

**Vòng đời pause:ask:**

```powershell
# 1. Chạy → clarify in ASK_USER: → engine dừng awaiting_input, exit 4
./run.ps1 run my-app "request" -Mock
# ⏸ Run dừng — node 'clarify' cần câu trả lời:
#   Câu hỏi: Bạn muốn màu sắc gì?

# 2. Xem câu hỏi (status)
./run.ps1 status my-app

# 3. Bơm câu trả lời → engine tiêm {{user_answer}} → re-run clarify → tiếp tục
./run.ps1 resume my-app -Answer "màu xanh"
```

- `exit 4` = run dừng tại `awaiting_input`. Không phải lỗi — tiếp bằng `resume -Answer`.
- Nếu agent vẫn in `ASK_USER:` sau re-run → pause lại (vòng hỏi, chặn bởi `max_steps`).
- `-AutoApprove` + `pause:ask` → fail-rõ (headless không thể tự trả lời).
- Demo: `examples/ask-demo/` (clarify pause:ask → build); `selftest` auto-verify mục `ask-demo/done-gate`.
- Demo: `examples/always-demo/` (work pause:always → report).
