...
|---|---|---|---|
| **PROJECT** | `run` | `run <proj> "<request>" [-Mock] [-Model m]` | Chạy pipeline end-to-end |
| | `resume` | `resume <proj> [-Mock] [-Model m]` | Tiếp tục run dở / `failed` mới nhất (cùng run dir) |
...
- **Xem output (A-15)**: lúc chạy, `run` chỉ in `node 'x' → done (N chars)` — **không** in nội dung. Dùng `logs <proj> [node]` SAU đó để đọc prompt/output đầy đủ từng lượt thăm. *(Stream output trực tiếp → Phase E.)*
...
`engine/e2e.ps1`: dry-run gate (mock, xác nhận GRAPH tới terminal **trước khi** đốt token) → sandbox cô lập → run thật → verify (`validate`/`check`) → promote branch đạt vào `projects/`. `autofix` thêm fix-loop: seed branch hỏng → assert pre-fix `validate` FAIL → HQ patch thật → assert post-fix exit 0 + `file_changed` → promote.
...
│   ├── workflow.ps1     # executor: single-cursor walk + router + loop + state + resume + frontmatter→CLI
│   ├── bridge.ps1       # resolve {{key}} + {{mem_*}} → prompt
│   ├── viz.ps1          # graph → ASCII + Mermaid (lệnh: graph)
│   ├── validate.ps1     # graph validation
...
│   ├── e2e.ps1          # Real-run harness HQ (lệnh: autobuild / autofix)
│   ├── test-runner.ps1  # Invoke-SelfTest (lệnh: selftest) — gom 3 script + 7 stamp + mem-demo
│   ├── pattern.ps1      # Expand-Pattern (stamp fragment)
│   ├── memory.ps1       # Get-Memory / Write-MemoryEntry
│   ├── status.ps1       # status + logs viewer
│   ├── edit.ps1         # TUI
...
**Trạng thái build**: toàn bộ Phase R / 0 / 1 / 2 / 3 / 4 / 5 / M đã ✅ DONE (xem [`plan/hq-build/ROADMAP.md`](plan/hq-build/ROADMAP.md)). Phase 6 (app GUI local) là tuỳ chọn, chưa làm.

Bộ test offline (mock, không đốt token): `examples/hq-tests.ps1` (per-agent HQ), `examples/hq-graph-tests.ps1` (8 path HQ graph), `examples/e2e-harness-tests.ps1` (harness round-trip). Chạy gom tất cả qua **`./run.ps1 selftest`** (3 script + 7 `p-*/stamp.ps1` + mem-demo done-gate; exit = số mục fail).
