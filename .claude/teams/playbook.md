...
**cto** — plan markdown (paste) + research + stack gợi ý. STOP: mỗi Step có "Cách tiếp cận",
cấu trúc file cụ thể tại `projects/<name>/`, đủ để builder không phải đoán.

**builder** — plan + thiết kế CTO (paste) + tên project `<name>`. STOP: file ghi vào
`projects/<name>/`, smoke-check (build/syntax) pass, báo tester kèm cách chạy + done-criteria.

**tester** — done-criteria (từ plan) + cách chạy (từ builder) + tên project. STOP: chạy check
thật, in `CHECK_RESULT: pass|fail` kèm bảng criterion|evidence, ghi bài học memory.
...
| cto | Mỗi Step có "Cách tiếp cận"; cấu trúc file cụ thể; done-criteria verify được; văn xuôi KHÔNG build-spec |
| builder | File đã ghi `projects/<name>/`; smoke-check pass; báo cách chạy + trạng thái từng done-criteria |
| tester | `CHECK_RESULT` rõ; mỗi criterion có evidence (exit-code/output thật, KHÔNG cảm tính); đã ghi memory |
...
**Teammate:**
8. **Xuất JSON / build-spec / plan-as-data** (planner/cto) — tàn dư workflow cũ, không engine nào parse.
...
12. **Build qua `run.ps1 autobuild`** (builder) — engine-build đã loại khỏi HQ; ghi file trực tiếp Write/Edit.
...
File: `.claude/team-issues-queue.md`. Khác `mistakes.md` (code bug) / `patterns.md` (code pattern):
...
## 10. Build-deliverable contract

> Chi tiết: skill `build-verify` (`.claude/skills/build-verify/SKILL.md`).

**Builder** — output `projects/<name>/` (gitignored, regen-được, không commit). Write + Edit +
Bash (cài deps, chạy build kiểm syntax). KHÔNG `run.ps1 autobuild/build`, KHÔNG `workflow.json`
HQ, KHÔNG đụng `engine/*.ps1`. Xong → báo tester kèm cách chạy + done-criteria.

**Tester** — chạy check của chính deliverable (`npm test`/`npm run build`/`pytest`/`go test`/
lint/quan sát hành vi). Nguồn sự thật = exit-code + output. In `CHECK_RESULT: pass|fail`. Ghi
memory sau verify.

**Engine = công cụ đứng riêng** — `run.ps1` + `app/` cho workflow CHI-NHÁNH. Lead/builder CHỈ
gọi khi request **cụ thể là dựng/sửa workflow pipeline** (ngoại lệ). Luồng build HQ mặc định =
Write/Edit trực tiếp.
